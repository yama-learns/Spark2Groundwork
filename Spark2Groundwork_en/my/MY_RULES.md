# This project's working rules (**my rules**)

> 🔴 **This file is ⛔ NOT part of the framework. An upgrade will not overwrite it.**
> ⚠️ **Day to day, this is the only rule file you need** — every framework rule is in §1,
> your own rules are in §2.

---

## 0. Two numbering systems, ⛔ never shared

| Number | Whose | Who changes it |
|---|---|---|
| **`R-xx`** | **The framework's** | ⛔ **Arrives only through an upgrade. ⛔ Do not rewrite its text here** |
| **`P-xx`** | 🔴 **Yours** | You add it after you have adjudicated it |

🔴 **⛔ A project rule must NOT continue the sequence as `R-36`.**
⚠️ **The next framework version may add an `R-36` of its own — ⛔ two different rules would
then share one number across projects, while `governance/RULES.md` opens with
"permanent numbering, append-only, never reused".**

### What if you want to change a framework rule

| Move | How |
|---|---|
| **Tighten** (recommended) | Add a `P-xx` whose text says "this tightens `R-05`" |
| **Override** (exception) | 🔴 **On a new line below that rule's text**, write `[project override] <reason>` (⛔ never on the `**R-xx**` line itself, never blank), and register it in §3 |

**The checker does ⛔ not fail on `[project override]`; it prints "N overrides" and names each one —**
⚠️ **an exemption is allowed, ⛔ but an exemption has to be visible.**

### What to do when the checker reports `RULE_MISSING_IN_MY` after an upgrade

**Run this. It copies the framework's new rule text into §1 verbatim and prints what it added:**

```
python scripts/harness/tool_sync_my_rules.py
```

⛔ **⛔ Do not retype it from memory** — ⚠️ **that step is exactly where "two copies, only one updated" comes from.**

---

## 1. Framework rules (**copied verbatim from `governance/RULES.md`, ⛔ do not rewrite**)
...
<!-- FRAMEWORK_RULES_END ⛔ 這一行是 tool_sync_my_rules.py 的插入點，不要刪 -->

---

## 2. This project's own rules (`P-xx`)

**(None yet. ⚠️ This "none" is the result of taking stock, ⛔ not a section nobody filled in.)**

> **How to write a `P-xx`:**
> **Start the line with `**P-01**`, then one executable checking action.**
> ⛔ **If it cannot be written as a concrete action, ⛔ do not make it a rule** —
> it reads like a reminder and stops nothing.

---

## 3. Override register

| `R-xx` | Reason for the override | Date |
|---|---|---|
| — | *(none yet)* | — |
