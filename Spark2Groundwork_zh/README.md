...
| 做什麼         | 在哪裡                                                               |
| -------------- | -------------------------------------------------------------------- |
| **填一個檔案** | 資料夾裡的 `PROJECT.md`。⛔ 其他治理文件都由 AI 維護，你不必打開     |
| **按三個按鈕** | 記錄快照／查看變更／檢查更新（Windows 用 `.bat`，Mac 用 `.command`） |
| **做決定**     | AI 提出建議，你決定要不要採納。**這一步不能外包**                    |

⚠️ **框架有新版時可以整包換掉，⛔ 而你的資料——你的構想、你的引用、你放進去的論文——永遠不會被動到。**
...
你這個專案自己出過的事，記在 `incidents/MY_INCIDENTS.md`。**
...
| 你做什麼 | 框架長出什麼 |
|---|---|
| 發現 AI 做錯了一件事，說一句「記下來」 | `incidents/MY_INCIDENTS.md` 多一筆 |
| 累積幾筆之後，請 AI 找出重複的模式 | 你看見一個**會再犯**的形狀 |
| 你決定要不要把它變成規則 | `governance/RULES.md` 多一條，**而且是你這個專案專用的** |
...
🔴 **升級工具只認得框架自己的那九個名字，其餘一律拒絕。**
**所以你的台帳、語料、事故簿、`PROJECT.md`，
以及⛔ 任何你自己開的資料夾，都不會被碰**——
**你不需要事先把它們登記在任何清單上。**
